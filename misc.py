...
def add_item():
    ...
    if form.validate_on_submit():
        ...
        image.save(os.path.join(app.static_folder, 'images/items', f'{filename_1}'))
        filename_2 = os.path.basename(form.img_url_2.data.filename)
        image = Image.open(form.img_url_2.data)
        image.save(os.path.join(app.static_folder, 'images/items', f'{filename_2}'))
        new_item = Item(
            name=form.name.data,
...
def store():
    ...
    for item in items:
        image = Image.open(f"static/images/items/{item.img_url_1}")
        image.thumbnail((200, 200))
        image.save(f"static/images/store/{item.img_url_1}")
...
def show_item(item_id):
    ...
    image = Image.open(f"static/images/items/{requested_item.img_url_1}")
    image.thumbnail((400, 400))
    image.save(f"static/images/large_size/{requested_item.img_url_1}")
    image = Image.open(f"static/images/items/{requested_item.img_url_2}")
    image.thumbnail((400, 400))
    image.save(f"static/images/large_size/{requested_item.img_url_2}")
...
def show_cart():
    ...
    for item in cart_items:
        image = Image.open(f"static/images/items/{item.img_url}")
        image.thumbnail((100, 100))
        image.save(f"static/images/cart/{item.img_url}")
        subtotal += item.price
